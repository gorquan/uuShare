...
HttpConnection* HttpManager::download(string url, OutputStream* stream,  HttpManager::CallBack callback, const string& userAgent) {
	auto conn = makeConn(move(url), useCoral(url), stream, callback, userAgent);
	conn->download();
	return conn;
}
...
HttpConnection* HttpManager::download(string url, const string& file, HttpManager::CallBack callback, const string& userAgent) {
	auto conn = makeConn(move(url), useCoral(url), createFile(file), callback, userAgent, HttpManager::CONN_MANAGED | HttpManager::CONN_FILE);
	conn->download();
	return conn;
}
...
bool HttpManager::useCoral(const string& url) {
	bool useCoral = BOOLSETTING(CORAL);
	// Prevent coral use on internal URI's
	if(useCoral && url.find(".apexdc.net") != string::npos)
		useCoral = false;
	return useCoral;
}
